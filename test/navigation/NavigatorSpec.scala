...
class NavigatorSpec extends SpecBase {
  ...
  val navigator = new Navigator(config)
  ...
  "Navigator" - {
    ...
    "in Normal mode" - {
      ...
      "must go from DirectDebitSourcePage to PaymentPlanTypePage if source is MGD, TC" in {
        val validSources = Seq(MGD, TC)
      ...
      "must go from DirectDebitSourcePage to PaymentReferencePage if source is CT, NIC, OL, PAYE, SDLT, VAT, SA" in {
        val validSources = Seq(CT, NIC, OL, PAYE, SDLT, VAT, SA)
      ...
      "must go from PaymentReferencePage to PaymentAmountController for CT, NIC, OL, SDLT, VAT, SA" in {
        val validSources = Seq(CT, NIC, OL, SDLT, VAT, SA)
      ...
      "must go from PaymentReferencePage to PaymentAmountController for MGD, TC with SinglePaymentPlan" in {
        val sources = Seq(MGD, TC)
...
// TODO: delete this whole spec when feature flag is removed
class NavigatorSaBppEnabledSpec extends SpecBase {
