...
import utils.{ClockProvider, DirectDebitDetailsData}
...
class NationalDirectDebitServiceSpec extends SpecBase with MockitoSugar with DirectDebitDetailsData {
  ...
  val service = new NationalDirectDebitService(mockConnector, mockCache, mockConfig, ClockProvider(currentClock))
  ...
  val testBankDetailsAuddisFalse: YourBankDetailsWithAuddisStatus =
    YourBankDetailsWithAuddisStatus(accountHolderName = testAccountHolderName,
                                    sortCode          = testSortCode,
                                    accountNumber     = testAccountNumber,
                                    auddisStatus      = false,
  ...
  "NationalDirectDebitService" - {
    ...
    "getFutureWorkingDays" - {
      "should use TEN_WORKING_DAYS for MGD variable payment plan" in {
        val userAnswers =
          emptyUserAnswers
      ...
      "should add TWO_WORKING_DAYS for new direct debit setup for auddis disabled" in {
        ...
        when(mockConfig.paymentDelayDynamicAuddisNotEnabled).thenReturn(8)
        when(mockConfig.TWO_WORKING_DAYS).thenReturn(2)
      ...
      "should calculate earliest date for add flow" in {
        ...
        val submissionDate = LocalDate.now().minusDays(10)

        when(mockCache.getDirectDebit(any())(any()))
          .thenReturn(Future.successful(nddResponse.directDebitList.head))
        when(mockConnector.getFutureWorkingDays(WorkingDaysOffsetRequest(submissionDate.toString, 3)))
          .thenReturn(Future.successful(EarliestPaymentDate("2025-02-18")))
        when(mockConfig.paymentDelayDynamicAuddisEnabled).thenReturn(5)
        when(mockConfig.TWO_WORKING_DAYS).thenReturn(2)
        when(mockConfig.THREE_WORKING_DAYS).thenReturn(3)
        when(mockConnector.getFutureWorkingDays(any())(any()))
      ...
      "fail when the connector call fails" in {
        ...
        val result = intercept[Exception](service.getFutureWorkingDays(expectedUserAnswers, "123").futureValue)
    ...
    "retrieveDirectDebitPaymentPlans" - {
      "must successfully return the direct debit payment plans from cache" in {
        ...
        val ddPaymentPlanResponse = NddDDPaymentPlansResponse(
          bankSortCode      = bankSortCode,
          bankAccountNumber = bankAccountNumber,
          bankAccountName   = bankAccountName,
          auDdisFlag        = "01",
        ...
        when(mockConnector.retrieveDirectDebitPaymentPlans(any())(any()))
          .thenReturn(Future.successful(ddPaymentPlanResponse))

        val result = service.retrieveDirectDebitPaymentPlans("userId", ddReference).futureValue

        result mustBe ddPaymentPlanResponse.copy(auDdisFlag = "false")
      }

      "must successfully return the direct debit payment plans from the api and update the cache" in {
        val bankAccountNumber = "12345678"
        val bankAccountName = "MyBankAcc"
        val bankSortCode = "123456"
        val ddReference = "122222"
        ...
        val updatedDbObject: NddResponse = NddResponse(
          directDebitCount = 1,
          directDebitList = Seq(
            NddDetails(
              ddiRefNumber       = ddReference,
        ...
        when(mockCache.retrieveCache(any()))
          .thenReturn(Future.successful(ddListResponse.directDebitList))
        when(mockCache.updateDirectDebit(any(), any())(any()))
          .thenReturn(Future.successful(updatedDbObject.directDebitList.head))
        when(mockConnector.retrieveDirectDebitPaymentPlans(any())(any()))
          .thenReturn(Future.successful(ddPaymentPlanResponse))

        val result = service.retrieveDirectDebitPaymentPlans("userId", ddReference).futureValue

        result mustBe ddPaymentPlanResponse
      }

      "must successfully return the direct debit empty payment plans from the cache" in {
        val bankAccountNumber = "12345678"
        val bankAccountName = "MyBankAcc"
        val bankSortCode = "123456"
        val ddReference = "122222"
        ...
        result mustBe NddDDPaymentPlansResponse(
          bankSortCode      = bankSortCode,
          bankAccountName   = bankAccountName,
          bankAccountNumber = bankAccountNumber,
          auDdisFlag        = "false",
  ...
  "calculateNextPaymentDate" - {
    ...
    "when planStartDate is beyond three working days then potentialNextPaymentDate should return startDate" in {
      ...
      result.potentialNextPaymentDate.value.equals(startDate) mustBe true
      result.nextPaymentDateValid mustBe true
    }
    ...
    "when planStartDate is within three working days then potentialNextPaymentDate is after planEndDate nextPaymentDateValid must be false" in {
      val today = LocalDate.of(2026, 1, 25)
      ...
      result.potentialNextPaymentDate.value.isAfter(planEndDate) mustBe true
    ...
    ".forWeeklyFrequency" - {
      ".frequency is Weekly" - {
        ...
        "when planStartDate is prior to next three working days but potentialNextPaymentDate is not within 3 working days" in {
          ...
          result.potentialNextPaymentDate mustBe Some(startDate.plusWeeks(0))
          result.nextPaymentDateValid mustBe true
        }
      ...
      ".frequency is FourWeekly" - {
        ...
        "when planStartDate is a past date but potentialNextPaymentDate is within 3 working days" in {
          ...
          when(mockConnector.getFutureWorkingDays(any())(any()))
            .thenReturn(Future.successful(EarliestPaymentDate(startDate.plusWeeks(4).plusDays(3).toString)))
      ...
      ".frequency is Monthly" - {
        "when planStartDate is within 3 working days" - {
          ...
          "but potentialNextPaymentDate is not within 3 working days" in {
            ...
            result.potentialNextPaymentDate.value.isAfter(startDate) mustBe true
            result.nextPaymentDateValid mustBe true
          }
        ...
        "when planStartDate is a past date" - {
          ...
          "but planStartDate is same calendar year and potentialNextPaymentDate is not within 3 working days" in {
            ...
            val nddService = new NationalDirectDebitService(mockConnector, mockCache, mockConfig, ClockProvider(fixedClock))
