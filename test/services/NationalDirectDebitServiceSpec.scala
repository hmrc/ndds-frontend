...
class NationalDirectDebitServiceSpec extends SpecBase with MockitoSugar with DirectDebitDetailsData {
  ...
  "calculateNextPaymentDate" - {
    ...
    "must return start date when start date is after today and beyond 3 working days (Step 1.1)" in {
      val startDate = fixedToday.plusDays(10)
      val planEndDate = fixedToday.plusMonths(3)

      when(mockConnector.getFutureWorkingDays(any())(any()))
        .thenReturn(Future.successful(EarliestPaymentDate(fixedToday.toString)))
    ...
    "must calculate next payment date when start date is within next 3 working days (Step 1.2 triggered)" in {
      val startDate = fixedToday.plusDays(1)
      val planEndDate = fixedToday.plusMonths(3)
      ...
      result.potentialNextPaymentDate.isDefined mustBe true
      result.potentialNextPaymentDate.get.isAfter(startDate) mustBe true
      result.nextPaymentDateValid mustBe true
    }

    "must calculate weekly next payment date correctly when plan already started" in {
      val startDate = fixedToday.minusWeeks(2)
      val planEndDate = fixedToday.plusWeeks(6)
    ...
    "must calculate monthly next payment date correctly when plan already started" in {
      val startDate = fixedToday.minusMonths(4)
      val planEndDate = fixedToday.plusMonths(12)
    ...
    "must set nextPaymentDateValid = false when potential next payment date is after plan end date (Step 2 validation)" in {
      val startDate = fixedToday.minusWeeks(2)
      val planEndDate = fixedToday.minusDays(1) // plan ended yesterday
    ...
    "must set nextPaymentDateValid = true when plan end date is None" in {
      val startDate = fixedToday.minusWeeks(2)

      when(mockConnector.getFutureWorkingDays(any())(any()))
        .thenReturn(Future.successful(EarliestPaymentDate(fixedToday.toString)))

      val result = service.calculateNextPaymentDate(startDate, None, Weekly).futureValue
