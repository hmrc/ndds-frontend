  ...
  "SubmitDirectDebitPaymentPlan" - {
    "paymentPlanTypeWrites method" - {
      "must write PaymentPlanType correctly" in {
        ...
        testCases.foreach { case (planType, expectedString) =>
          val model = SubmitDirectDebitPaymentPlan("testReference", planType)
          val json = Json.toJson(model)

          (json \ "paymentReference").as[String] mustBe "testReference"
          (json \ "planType").as[String] mustBe expectedString
        }
      }
    }
  }
