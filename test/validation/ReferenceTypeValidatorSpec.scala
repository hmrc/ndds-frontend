...
class ReferenceTypeValidatorSpec extends AnyFreeSpec with Matchers {
  ...
  "ReferenceType Validator" - {
    ...
    "must fail to validate PAYE reference with incorrect format" in {
      val validation = summon[ReferenceTypeValidator.Validator[PAYE.type]]
      val invalidPAYEReference = Seq("A961PX0023480X", "ABCPD0020230X", "861PH0020230X", "961PX0Y234809", "961PX1023480X", "abcpD0020230x")

      invalidPAYEReference.foreach(ref => validation.validate(ref) mustEqual false)
    }
    ...
    "must validate MGD reference with correct format successfully" in {
      ...
      val validMGDReference = Seq("XVM00005554321", "xvm00005554321", "xVm00005554321", "XYM00000001536", "xym00000001536")
      validMGDReference.foreach(ref => validation.validate(ref) mustEqual true)
    }
    ...
    "must fail to validate MGD reference with incorrect format" in {
      ...
      invalidMGDReference.foreach(ref => validation.validate(ref) mustEqual false)
    }
    ...
    "must fail to validate SDLT reference with incorrect format " in {
      ...
      validSDLTReference.foreach(ref => validation.validate(ref) mustEqual false)
    }
    ...
    "must validate VAT reference with correct format successfully" in {
      ...
      val validVATReference = Seq("562235945",
                                  "100212755",
                                  "959939732",
                                  "562235987",
                                  "232457280",
      ...
      validVATReference.foreach(ref => validation.validate(ref) mustEqual true)
    }

    "must invalidate VAT reference with incorrect format" in {
      val validation = summon[ReferenceTypeValidator.Validator[VAT.type]]
      val invalidVATReference = Seq("562235946",
                                    "100212756",
                                    "959939832",
                                    "562235988",
                                    "232457281",
      ...
      invalidVATReference.foreach(ref => validation.validate(ref) mustEqual false)
    }
    ...
    "must validate NIC reference with correct format successfully" in {
      ...
      val validNICReference = Seq("600340016213526259", "600340016213536354", "601340016113236351", "601340014200236351", "601340010200236358")

      validNICReference.foreach(ref => validation.validate(ref) mustEqual true)
    }

    "must fail to validate NIC reference with incorrect format successfully" in {
      val validation = summon[ReferenceTypeValidator.Validator[NIC.type]]
      val invalidNICReference = Seq("600340113213526259", "600340016213536357", "601340016113236451", "601340014200236451", "601340010200234358")

      invalidNICReference.foreach(ref => validation.validate(ref) mustEqual false)
    }
    ...
    "must validate Tax Credit reference with correct format successfully" in {
      ...
      val validTaxCreditReference = Seq("WT447571311207NE", "Wt447571311207NE", "wt447571311207ne")

      validTaxCreditReference.foreach(ref => validation.validate(ref) mustEqual true)
    }
