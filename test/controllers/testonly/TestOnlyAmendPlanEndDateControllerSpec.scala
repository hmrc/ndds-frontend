...
class TestOnlyAmendPlanEndDateControllerSpec extends SpecBase with MockitoSugar {
  private implicit val messages: Messages = stubMessages()
  private val formProvider = new AmendPlanEndDateFormProvider()
  private def form = formProvider()
  val validAnswer: LocalDate = LocalDate.now()

  lazy val amendPlanEndDateRoute: String = testonlyRoutes.TestOnlyAmendPlanEndDateController.onPageLoad(NormalMode).url
  lazy val amendPlanEndDateRoutePost: String = testonlyRoutes.TestOnlyAmendPlanEndDateController.onSubmit(NormalMode).url
