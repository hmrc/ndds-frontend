...
class RegularPaymentAmountControllerSpec extends SpecBase with MockitoSugar {
  ...
  "RegularPaymentAmount Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, regularPaymentAmountRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(RegularPaymentAmountPage, validAnswer).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(validAnswer), NormalMode, Call("GET", paymentsFrequencyRoute))(request,
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, regularPaymentAmountRoute)
