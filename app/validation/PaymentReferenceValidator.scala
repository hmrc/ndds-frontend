...
import utils.ModUtils.{charRemainderMap, modCheck, safe14ModCheck}

import java.time.LocalDate
import java.time.format.DateTimeFormatter
import scala.util.{Failure, Success, Try}
object PaymentReferenceValidator {
...
//  }
  ...
  private[validation] def isValidSafe15Ref(ref: String): Boolean = {
    ...
    val protectedChars = ref.substring(indexOfFirstProtectedChar, indexOfLastProtectedChar)
    try {
      val expectedChar = modCheck(modDivisor, weightings, charRemainderMap, ModUtils.parseIntArray(protectedChars))
